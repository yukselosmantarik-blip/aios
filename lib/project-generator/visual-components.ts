...
function generateMediaPlaceholder(): string {
  return [
    ...
    "        <div className=\"flex h-full w-full items-center justify-center p-[var(--spacing-md)]\">",
    "          <Placeholder label={media.label} category=\"image\" />",
    "        </div>",
    "      </MediaFrame>",
    "      <figcaption className=\"mt-2 text-[length:var(--font-size-sm)] text-[var(--color-text-muted)]\">",
...
function generateSiteHeader(project: CompiledWebsiteProject): string {
  ...
  return [
    ...
    '            <Link href="/" className="text-lg font-[var(--font-weight-semibold)] focus-visible:outline-none">',
    `              ${JSON.stringify(project.business.businessName)}`,
    "            </Link>",
    "            <Placeholder label=" + JSON.stringify(placeholderLabel("Logo")) + " category=\"logo\" className=\"sr-only\" />",
    '            <button',
    '              type="button"',
...
function generateHeroSection(project: CompiledWebsiteProject): string {
  ...
  return [
    ...
    "              ) : null}",
    "            </Cluster>",
    "            <MediaPlaceholder media={{ id: `${section.id}-media`, label: mediaLabel, altText: mediaLabel }} />",
    "          </Stack>",
    "        ) : heroVariant === 'split' ? (",
    ...
    "              </Cluster>",
    "            </Stack>",
    "            <MediaPlaceholder media={{ id: `${section.id}-media`, label: mediaLabel, altText: mediaLabel, aspectRatio: '4/3' }} />",
    "          </div>",
    "        ) : heroVariant === 'full-bleed' ? (",
    '          <Stack gap="lg">',
    '            <div className="relative overflow-hidden rounded-[var(--radius-lg)]">',
    "              <MediaPlaceholder media={{ id: `${section.id}-media`, label: mediaLabel, altText: mediaLabel, aspectRatio: '16/9' }} />",
    ...
    "            <MediaPlaceholder media={{ id: `${section.id}-media`, label: mediaLabel, altText: mediaLabel, aspectRatio: '1/1' }} />",
    '            <Stack gap="lg">',
...
function generateProductGridSection(project: CompiledWebsiteProject): string {
  ...
  return [
    ...
    '                    <Stack gap="sm">',
    "                      <MediaPlaceholder media={{ id: `${item.id}-media`, label: " + JSON.stringify(placeholderLabel("Produktbild")) + ", altText: item.name, aspectRatio: '4/3' }} />",
    '                      <div className="flex items-start justify-between gap-[var(--spacing-sm)]">',
...
function generateGallerySection(): string {
  ...
  return generatePremiumSectionWrapper("GallerySection", [
    ...
    "                    <MediaPlaceholder media={{ id: `${section.id}-media-${index}`, label: item, altText: item, aspectRatio: index % 2 === 0 ? '4/3' : '3/4' }} />",
    "                  </div>",
    "                ))}",
    "              </div>",
    "            ) : (",
    ...
    "                    <MediaPlaceholder media={{ id: `${section.id}-media-${index}`, label: item, altText: item }} />",
    "                  </li>",
    "                ))}",
    "              </ResponsiveGrid>",
    "            );",
...
function generateTestimonialSection(): string {
  return generatePremiumSectionWrapper("TestimonialSection", [
    ...
    "          </Card>",
  ], ["import { Card } from './Card';", "import { Placeholder } from './Placeholder';"]);
}
...
function generateMapSection(): string {
  return [
    ...
    "  return (",
    "    <div",
    '      className={cn(variants.placeholder, "flex min-h-48 items-center justify-center rounded-[var(--radius-lg)]")}',
    '      role="img"',
    "      aria-label=" + JSON.stringify(placeholderLabel("Map")) + "",
    "    >",
...
function generateContentSection(): string {
  return generatePremiumSectionWrapper("ContentSection", [
    ...
    "            <MediaPlaceholder media={{ id: `${section.id}-media`, label: section.media[0], altText: section.media[0] }} />",
    "          ) : null}",
    "          {section.primaryCTA ? (",
    "            <ButtonLink cta={{ label: section.primaryCTA, href: '/', variant: 'primary' }} />",
    "          ) : null}",
